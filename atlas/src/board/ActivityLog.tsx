...
function describe(a: JobActivity): string {
  ...
  switch (a.action) {
    ...
    case 'stage_changed':
      return `Moved ${stageLabel(d.from as string)} → ${stageLabel(d.to as string)}`;
    case 'archived':
      return 'Archived';
    case 'unarchived':
      return 'Unarchived';
...
export function ActivityLog({
  ...
  return (
    <section className="panel">
      <h3>Activity</h3>
      {rows.length === 0 && <p className="muted small">No activity yet.</p>}
      <ul className="timeline">
        {rows.map((a) => (
          <li key={a.id}>
            <span className="t-what">{describe(a)}</span>
            <span className="muted small">{dateTimeLabel(a.created_at)}</span>
            <span className="t-meta">by {profileName(a.actor)}</span>
          </li>
        ))}
      </ul>
    </section>
