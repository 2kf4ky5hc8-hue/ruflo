...
): Promise<{ ok: boolean; inserted: number; skipped: number; isaMirrored: number; error?: string }> {
  const session = await auth();
  if (!session?.user) return { ok: false, inserted: 0, skipped: 0, isaMirrored: 0, error: 'Not signed in.' };
  const userId = (session.user as { id: string }).id;
  try {
    const parsed = detectAndParse(csvText);
    if (parsed.rows.length === 0) return { ok: false, inserted: 0, skipped: 0, isaMirrored: 0, error: 'No rows to import.' };
    const res = await commitImportedRows(userId, accountId, parsed.rows);
    return { ok: true, inserted: res.inserted, skipped: res.skipped, isaMirrored: res.isaMirrored };
  } catch (e) {
    return { ok: false, inserted: 0, skipped: 0, isaMirrored: 0, error: (e as Error).message };
  }
}
